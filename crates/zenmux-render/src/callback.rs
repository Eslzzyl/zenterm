...
//!
//! Bridges the CPU-side terminal grid data (built in [`zenmux_ui::ZenmuxApp`])
//! with the GPU render pass via egui's custom callback mechanism.
//!
//! # Lifecycle
//!
//! 1. [`TerminalWgpuCallback`] is created once in `ZenmuxApp` and stored
//!    behind an `Arc<CallbackHandle>`.
//! 2. Each egui frame, `ZenmuxApp` builds `Vec<CellInstance>` from visible
...
/// Thread-safe shared state between `ZenmuxApp` (updates each frame) and
