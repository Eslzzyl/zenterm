...
### ZT-02：恢复布局时产生隐藏会话；单终端模式可能向隐藏会话发送键盘输入（代码已确认）

启动时先无条件创建会话 0，再根据 `dock.json` 创建保存的会话。如果保存布局不含 0 且活动工作区已有标签，会话 0 留在会话池中运行却没有标签。恢复逻辑也没有以 `tabs_enabled` 为条件：关闭多标签模式后，界面固定绘制会话 0，键盘却发送给恢复布局选出的活动会话，二者可能不同。

依据：[创建与恢复](crates/zenterm-ui/src/app/mod.rs#L182-L292)、[单终端绘制](crates/zenterm-ui/src/legacy.rs#L27-L35)、[键盘路由](crates/zenterm-ui/src/app/keyboard.rs#L13-L16)。建议围绕“会话 0 已关闭后重启”和“多标签模式改为关闭后重启”增加恢复测试。

### ZT-03：Kitty 临时文件传输可删除由终端输出指定的本地文件（代码已确认）

`t=t` 接受输出数据中的本地路径。读取成功后，路径只要满足 `/tmp/`、`/var/tmp/`、`/dev/shm/` 或 `TMPDIR` 的字符串前缀条件，就会尝试删除。远程命令或文件内容可产生终端输出，因此这里存在跨越 PTY 输出信任边界的本地文件操作。

依据：[协议解析](crates/zenterm-term/src/image/kitty.rs#L52-L70)、[读取与删除](crates/zenterm-term/src/image/kitty.rs#L75-L122)。建议单独审定临时文件所有权与删除策略，并覆盖路径前缀、符号链接和远程输出场景。
