...
Section Install
  ...
  ; Create the desktop shortcut in silent/passive mode only when explicitly
  ; requested with the /DS command-line option.
  IfSilent check_ds_flag 0
  ${IfThen} $PassiveMode == 1 ${|} Goto check_ds_flag ${|}
  Goto shortcuts_done
  check_ds_flag:
    ${GetOptions} $CMDLINE "/DS" $R0
    IfErrors shortcuts_done
...
Function CreateDesktopShortcut
  CreateShortcut "$DESKTOP\${PRODUCTNAME}.lnk" "$INSTDIR\${MAINBINARYNAME}.exe" "" "$INSTDIR\zenterm.ico" 0
  ApplicationID::Set "$DESKTOP\${PRODUCTNAME}.lnk" "${IDENTIFIER}"
FunctionEnd

Function CreateStartMenuShortcut
  CreateDirectory "$SMPROGRAMS\$AppStartMenuFolder"
  CreateShortcut "$SMPROGRAMS\$AppStartMenuFolder\${PRODUCTNAME}.lnk" "$INSTDIR\${MAINBINARYNAME}.exe" "" "$INSTDIR\zenterm.ico" 0
